...
for fp in task_files:
    ...
    # Find the forward declarations that became unused.
    content = re.sub(r'static\s+unsigned\s+int\s+ws63_[a-zA-Z0-9_]+_lock\s*\(\s*void\s*\)\s*;', '', content)
    content = re.sub(r'static\s+void\s+ws63_[a-zA-Z0-9_]+_unlock\s*\(\s*unsigned\s+int[^\)]*\)\s*;', '', content)
    
    # Did the previous script only replace ONE pair of lock functions?
    # Yes, `re.search` finds one. We need a `while` loop for multiple.
    while True:
        lock_match = re.search(r'static\s+unsigned\s+int\s+(ws63_[a-zA-Z0-9_]+_lock)\s*\(\s*void\s*\)\s*\{', content)
