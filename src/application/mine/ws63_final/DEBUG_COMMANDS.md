...
- `LD2401 LOG ON|OFF`（或 `LD2402 LOG ON|OFF`）
  - 功能：开启或关闭 LD2402 运行态日志输出。

- `LD2401 LOGINT <0-60000>`（或 `LD2402 LOGINT <0-60000>`）
  - 功能：设置 LD2402 运行态日志最小输出间隔（毫秒）。
  - 说明：`0` 表示每包都打印；推荐联调阶段使用 `500~2000`。

- `LD2401 LOGSTAT`（或 `LD2402 LOGSTAT`）
...
14. 若 LD2402 上电后日志过密，优先执行 `LD2401 LOGINT 1000`，必要时执行 `SLE ULOG OFF`。
...
- 现象：LD2402 一上电日志持续刷屏。
  - 排查：先执行 `LD2401 LOGSTAT` 确认运行态日志开关与间隔配置。
  - 处置：建议 `LD2401 LOGINT 1000`（或更大）；若仍需进一步降噪，可执行 `SLE ULOG OFF`。
