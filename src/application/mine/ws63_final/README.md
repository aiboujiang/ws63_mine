...
8. `ws63_zw101_task_start()` / `ws63_task_zw101_request_auto_identify()`：门锁接近窗口内自动触发指纹识别任务。
...
- 新增门锁编排任务骨架：按 LD2402 距离阈值进入接近窗口，并接收 camera 认证结果驱动开锁/锁定状态；认证窗口默认 20s，便于短暂停顿后继续完成输入或识别。
