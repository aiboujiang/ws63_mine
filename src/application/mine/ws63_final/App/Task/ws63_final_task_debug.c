...
static void ws63_debug_exec_command(const char *line)
{
    ...
    if (strcmp(cmd, "ZW LIST") == 0) {
        ...
        ret = ws63_task_zw101_read_index_table(0U, index_table, &zw_ack);
        if (ret == ERRCODE_SUCC) {
            char id_str[128] = {0};
            char tmp[16] = {0};
            ...
            ws63_debug_log("[ws63 dbg] ZW LIST ret=0x%x ack=0x%02x valid=%u IDs: %s\r\n",
                (unsigned int)ret,
                (unsigned int)zw_ack,
                (unsigned int)zw_valid_num,
                id_str);
        } else {
            ws63_debug_log("[ws63 dbg] ZW LIST failed ret=0x%x ack=0x%02x\r\n",
                (unsigned int)ret,
                (unsigned int)zw_ack);
        }
        return;
