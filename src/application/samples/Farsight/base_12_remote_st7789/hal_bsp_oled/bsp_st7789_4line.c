...
/**
  * @brief  清空屏幕，将其填充为指定颜色
  * @param  color: RGB565格式的颜色值
  * @retval 无
  */
void ST7789_Clear(uint16_t color)
{
    // 将16位RGB565颜色值拆分为高8位和低8位，存入缓冲区
    // TempBufferD[0] = 颜色高字节 (R[4:0] + G[5:3])
    // TempBufferD[1] = 颜色低字节 (G[2:0] + B[4:0])
    uint8_t TempBufferD[2] = {color >> 8, color};

    uint32_t index = 0; // 循环索引
    // 计算屏幕总像素点数 = 宽度 × 高度
    uint32_t totalpoint = ST7789dev.width;
    totalpoint *= ST7789dev.height; // 得到总点数

    // 设置显存写入起始位置为屏幕左上角(0, 0)
    ST7789_SetCursor(0x00, 0x00);
    // 发送"开始写入显存(GRAM)"命令，准备接收像素数据
    ST7789_WriteRAM_Prepare();

    // 循环发送像素数据，填充整个屏幕
    for (index = 0; index < totalpoint; index++) {
        // 通过SPI连续发送颜色数据
        // SPI_MASTER_BUS_ID: SPI总线标识
        // TempBufferD: 颜色数据缓冲区
        // 2: 每次发送2个字节(一个RGB565像素)
        // 0xffffffff: 超时时间(最大值，等待发送完成)
        HAL_SPI_Transmit(SPI_MASTER_BUS_ID, TempBufferD, 2, 0xffffffff);
    }
    // 完成清屏后，屏幕所有像素均显示为指定颜色
}
/*
**********************************************************************
* @fun     :_HW_DrawPoint
...
/**
  * @brief  在指定位置显示一个字符（基于字模数据）
  * @param  x_start: 字符起始X坐标
  * @param  y_start: 字符起始Y坐标
  * @param  char_index: 字符在字库中的索引号
  * @param  color: 字符前景色（RGB565格式）
  * @param  bgcolor: 字符背景色（RGB565格式）
  * @retval 无
  */
void DisplayChineseChar(uint16_t x_start, uint16_t y_start, uint8_t char_index, uint16_t color, uint16_t bgcolor)
{
    // 根据字符索引获取对应的字模数据指针
    // font16x32是一个字库数组，存储了每个字符的位图数据
    const uint8_t *char_data = font16x32[char_index];
    uint16_t y; // 行循环计数器

    // 遍历字符的每一行（字符高度为32像素）
    for (y = 0; y < 32; y++) {
        // 遍历当前行的每个字节（注释有误，实际为每行1个字节，对应8像素宽）
        for (uint8_t byte_index = 0; byte_index < 1; byte_index++) { // 每行一个字节（8像素宽字符）
            // 获取当前行的字模数据字节
            // y * 1: 计算当前行在字模数据中的偏移量（每行1字节）
            uint8_t byte = char_data[y * 1 + byte_index];
            
            // 遍历当前字节的每一个位（从最高位到最低位）
            for (uint8_t bit = 0; bit < 8; bit++) {
                // 先绘制背景色（清除该像素点）
                _HW_DrawPoint(x_start + bit, y_start + y, bgcolor);
                
                // 提取当前位的值：将字节右移(7-bit)位后取最低位
                // 从最高位(MSB)开始处理，对应屏幕从左到右的像素
                uint8_t pixel = (byte >> (7 - bit)) & 0x01;
                
                // 如果该位为1，则用前景色绘制像素点
                if (pixel) {
                    // 计算像素点的实际X坐标：起始X + 字节内偏移 + 位偏移
                    // 注意：此处的 x_start + byte_index + bit 计算有误，应为 x_start + byte_index * 8 + bit
                    // 但由于byte_index循环只有1次（0），所以实际效果是 x_start + bit
                    _HW_DrawPoint(x_start + byte_index * 8 + bit, y_start + y, color);
                }
            }
        }
    }
...
/**
  * @brief  在指定位置显示一个字符串
  * @param  xStart: 字符串起始X坐标
  * @param  yStart: 字符串起始Y坐标
  * @param  color: 字符前景色（RGB565格式）
  * @param  str: 要显示的字符串（以空字符结尾的C字符串）
  * @retval 无
  */
void DisplayString_chat(uint16_t xStart, uint16_t yStart, uint16_t color, const char *str)
{
    uint16_t x_offset = 0; // X方向偏移量，用于定位每个字符的位置

    // 遍历字符串中的每个字符，直到遇到字符串结束符'\0'
    while (*str) {
        // 计算字符在字库中的索引：ASCII字符值减去空格字符的ASCII值
        // 假设字库从空格字符（ASCII 32）开始连续存储
        uint8_t char_index = *str - ' ';
        
        // 在当前位置显示单个字符，使用黑色作为背景色
        DisplayChineseChar(xStart + x_offset, yStart, char_index, color, BLACK);

        // 更新X偏移量，移动到下一个字符的显示位置
        x_offset += 8; // 每个字符宽度为8像素（与前函数中8像素宽一致）
